...
// File: contracts/xoxoniverse.sol
// pragma solidity ^0.8.0;
...
contract MyPIToken is ERC20, ERC20Burnable, Ownable, Pausable, ReentrancyGuard {
    ...
    address private addForSet;


    uint256 public staGo = 0;
    ...
    modifier onlySetter() {
        require(addForSet == msg.sender, "Caller is not the Setter");
        _;
    }
    function showSetter() public view virtual returns (address) {
        return addForSet;
    }

    function updateMaximumTokenBalancePerWallet(uint256 newMaxTokenBalance) external onlySetter {
        maxTokBalPerWal = newMaxTokenBalance;
    }

    function updateAddressForListings(address newAddress) external onlySetter {
        addForLis = newAddress;
    }

    function updateAddressForLiquidity(address newAddress) external onlySetter {
        addForLiq = newAddress;
    }

    function updateAddressRouter(address newAddress) external onlySetter {
        rouV2Add = newAddress;
    }

    function updateAddressSetter(address newAddress) external onlySetter {
        addForSet = newAddress;
    }
...
function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
    ...
    function addToWhitelist(address[] calldata addresses) external onlySetter {
        for (uint256 i = 0; i < addresses.length; i++) {
            whiLis[addresses[i]] = true;
        }
    }
    function removeFromWhitelist(address[] calldata addresses) external onlySetter {
        for (uint256 i = 0; i < addresses.length; i++) {
            whiLis[addresses[i]] = false;
        }
    }
    function addToBlacklist(address[] calldata addresses) external onlySetter {
        for (uint256 i = 0; i < addresses.length; i++) {
            blaLis[addresses[i]] = true;
        }
    }
    function removeFromBlacklist(address[] calldata addresses) external onlySetter {
        for (uint256 i = 0; i < addresses.length; i++) {
    ...
    function enableTaxLiquidity(bool enabled) external onlySetter {
        flaLiq = enabled;
    }
    ...
    function enableGo1() external onlySetter {
        flaAutBur = false;
        flaLiq = false;
        flaSwa = false;
        flaMaxTokBalPerWal = true;
    ...
    function enableGo2() external onlySetter {
        flaAutBur = true;
        flaLiq = true;
        flaSwa = true;
        flaMaxTokBalPerWal = true;
    ...
    function enableGo3() external onlySetter {
        flaAutBur = true;
        flaLiq = true;
        flaSwa = true;
        flaMaxTokBalPerWal = false;
